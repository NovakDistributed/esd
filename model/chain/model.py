...
class AbstractWorld:
    """
    Interface for either a Python-simulated world or an actual testnet chain.
    
    Knows about on-chain state.
    
    Communicates with agents by fielding transactions and replying with events.
    """
    
    def enumerate_agents(self) -> Iterable[str]:
        """
        Yield strings identifying each agent in the world. Those agent IDs will
        need to be passed back for state queries or actions.
        """
        raise NotImplementedError
        
    def enumerate_objects(self) -> Iterable[str]:
        """
        Enumerate the things in the world that agents can act on (i.e.
        contracts or systems of contracts).
        
        Yields strings identifying objects that will need to be passed back to
        take actions.
        """
        raise NotImplementedError
        
    def enumerate_actions(self, obj: str) -> Iterable[ActionSpec]
        pass
    
    def attempt_action(self, agent: str, action: List) -> List:
        """
        Perform the given action on behalf of the given agent
        """
        raise NotImplementedError
